...
const LoginScreen: React.FC<Props> = ({ onLogin, hasApiKey, onOpenSettings }) => {
    ...
    const handleLogin = () => {
        ...
        if (!selectedStudent) {
            alert("Vui lòng chọn tên của bạn từ danh sách.");
            return;
        }
        ...
        <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-blue-50 to-white flex flex-col items-center justify-center p-6 font-sans relative">
            <div className="w-full max-w-lg space-y-6 animate-fade-in-up z-10 my-auto">
                ...
                <div className="text-center relative">
                    ...
                    <p className="text-base text-gray-500">Chọn tên của bạn để bắt đầu</p>
                </div>
                ...
                {!hasApiKey && (
                    <div
                        ...
                        className="bg-gradient-to-r from-red-50 to-orange-50 border-2 border-red-200 rounded-2xl p-4 cursor-pointer hover:shadow-lg transition-all group"
                    >
                        <div className="flex items-center gap-3">
                            <div className="w-12 h-12 rounded-xl bg-red-100 flex items-center justify-center text-red-500 group-hover:scale-110 transition-transform">
                                <i className="fas fa-key text-xl"></i>
                            </div>
                            <div className="flex-1">
                                <p className="font-bold text-red-700">Chưa có API Key!</p>
                                <p className="text-sm text-red-600">Nhấn vào đây để thiết lập API Key</p>
                            </div>
                ...
                <Card className="!p-0 !rounded-3xl border border-white/60 shadow-xl backdrop-blur-xl bg-white/80 overflow-visible">
                    ...
                    <div className="p-6 border-b border-gray-100 bg-white sticky top-0 z-10 rounded-t-3xl">
                        <label className="block text-sm font-bold text-gray-700 mb-2">Tìm tên của bạn</label>
                    ...
                    {/* List */}
                    <div className="h-[40vh] overflow-y-auto custom-scrollbar bg-transparent p-3 space-y-2">
                        {filteredStudents.length > 0 ? (
                            filteredStudents.map((student, idx) => (
                                <button
                                    key={idx}
                                    onClick={() => setSelectedStudent(student)}
                                    className={`w-full text-left px-4 py-3 rounded-2xl transition-all duration-200 flex items-center gap-3 border-2 ${selectedStudent === student
                                            ? 'bg-blue-50 border-blue-500 shadow-md ring-2 ring-blue-100'
                                            : 'bg-white border-transparent hover:bg-gray-50 hover:border-gray-200'
                                        }`}
                                >
                                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-bold text-lg transition-colors ${selectedStudent === student
                            ...
                            <div className="p-8 text-center text-gray-400">
                                <p className="text-sm font-medium">Không tìm thấy học sinh.</p>
                            </div>
                        )}
                    </div>
                    ...
                    <div className="p-6 bg-white border-t border-gray-100 rounded-b-3xl shadow-[0_-10px_40px_-15px_rgba(0,0,0,0.1)] relative z-20">
                        ...
                        >
                            {hasApiKey ? (
                                ...
                                <>Thiết lập API Key trước <i className="fas fa-key ml-2"></i></>
                            )}
                        </Button>
                        {hasApiKey && (
                            <button
                                onClick={onOpenSettings}
                                className="w-full mt-3 py-2 text-sm text-gray-500 hover:text-blue-600 font-medium transition-colors"
                            >
                                <i className="fas fa-cog mr-2"></i>
                                Thay đổi cài đặt API
                            </button>
                        )}
                    </div>
                </Card>
